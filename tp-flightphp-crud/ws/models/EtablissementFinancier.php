...
class EtablissementFinancier {
    ...
    public static function update($id, $data) {
        $db = getDB();
        error_log("DEBUG: EtablissementFinancier Model - Appel de update pour ID: " . $id);
        error_log("DEBUG: EtablissementFinancier Model - Données pour la mise à jour: " . print_r($data, true)); // Ceci affichera l'objet $data

        // La requête et l'exécution corrigées devraient être ici
        $stmt = $db->prepare("UPDATE EtablissementFinancier SET nom_etablissement = ?, adresse = ?, telephone = ?, email = ?, solde = ? WHERE id_etablissement = ?");
        $stmt->execute([$data->nom_etablissement, $data->adresse, $data->telephone, $data->email, $data->solde, $id]);

        error_log("DEBUG: EtablissementFinancier Model - Requête UPDATE exécutée.");
    }
    ...
    public static function addFunds($id_etablissement, $montant, $description = null) {
        ...
        try {
            ...
            if ($typeOperation) {
                ...
                $stmtOperation = $db->prepare("INSERT INTO OperationFonds (id_etablissement, id_type_operation, montant, description) VALUES (?, ?, ?, ?)");
